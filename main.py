...
def check_parametric_assumptions(data_list, names):
    report = []
    is_all_normal = True
    for data, name in zip(data_list, names):
        ...
        n = len(clean_data)
        if n < 3:
            report.append(f"⚠️ [{name}] 표본 수가 부족하여 검정이 불가합니다.")
            continue
        stat, p = stats.shapiro(clean_data)
        skew = clean_data.skew(); kurt = clean_data.kurt()
        if n >= 30:
            report.append(f"✅ [{name}] 표본 수(N={n})가 충분하여 중심극한정리에 의해 정규성을 가정함 (왜도:{skew:.2f}, 첨도:{kurt:.2f})")
        elif p >= 0.05:
            report.append(f"✅ [{name}] Shapiro-Wilk p={format_p(p)}로 정규성 가정을 충족함")
        else:
            report.append(f"⚠️ [{name}] Shapiro-Wilk p={format_p(p)}로 정규성 위배 가능성 있음")
            is_all_normal = False
    
    if len(data_list) >= 2:
        l_stat, l_p = stats.levene(*data_list)
        report.append(f"{'✅' if l_p >= 0.05 else '⚠️'} Levene 등분산 검정: p={format_p(l_p)}")
        return "\n".join(report), is_all_normal and (l_p >= 0.05)
    return "\n".join(report), is_all_normal
...
def create_pro_report(m_name, r_df, interpretation, guide, plot_b=None, assump=""):
    ...
    doc.add_heading(f'STATERA Analysis Report: {m_name}', 0).alignment = WD_ALIGN_PARAGRAPH.CENTER
    if assump:
        doc.add_heading('1. Assumption Checks', level=1)
        doc.add_paragraph(assump).italic = True
    ...
    if plot_b:
        doc.add_heading('3. Visualization', level=1); doc.add_picture(plot_b, width=Inches(4.5))
...
METHOD_GUIDES = {
    "기술통계": {"title": "📈 기술통계", "desc": "평균, 표준편차, 왜도, 첨도 등을 산출하여 데이터의 정규성을 파악합니다.", "독립": "N/A", "종속": "연속형"},
    "빈도분석": {"title": "📊 빈도분석", "desc": "범주형 변수의 빈도와 백분율을 산출하여 대상자 분포를 확인합니다.", "독립": "N/A", "종속": "범주형"},
    "카이제곱 검정": {"title": "🎲 카이제곱 및 Fisher", "desc": "범주형 변수 간 연관성 및 기대빈도 가정을 검정합니다.", "독립": "범주형", "종속": "범주형"},
    "T-검정": {"title": "👥 T-검정", "desc": "두 집단 간 평균 차이 및 정규성을 검정합니다.", "독립": "범주형(2집단)", "종속": "연속형"},
    "분산분석(ANOVA)": {"title": "🏫 ANOVA", "desc": "세 집단 이상 평균 차이 및 사후 검정을 수행합니다.", "독립": "범주형(3+)", "종속": "연속형"},
    "상관분석": {"title": "🔗 상관분석", "desc": "변수 간 선형적 관련성의 강도(Pearson's r)를 분석합니다.", "독립": "연속형", "종속": "연속형"},
    "신뢰도 분석": {"title": "📏 신뢰도 분석", "desc": "측정 도구의 내적 일관성(Cronbach's α)을 산출합니다.", "독립": "다수문항", "종속": "N/A"},
    "회귀분석": {"title": "🎯 회귀분석", "desc": "독립변수의 영향력, 모형 적합도, 오즈비(OR) 등을 분석합니다.", "독립": "연속/범주", "종속": "연속/이분형"}
}
...
if up_file:
    ...
    group = st.radio("분석 단계를 선택하세요", ["Step 1. 기초 분석", "Step 2. 차이 검정", "Step 3. 관계 및 신뢰도", "Step 4. 영향력 분석"], horizontal=True)
    ...
    if method == "기술통계":
        ...
        if st.button("실행"):
            ...
            assump_text, _ = check_parametric_assumptions([df[v]], [v])
            plt.figure(figsize=(10,4)); plt.subplot(1,2,1); sns.histplot(df[v].dropna(), kde=True); plt.subplot(1,2,2); stats.probplot(df[v].dropna(), plot=plt); plot_img = get_plot_buffer()
        ...
        if st.button("실행") and vs:
            ...
            for c in vs:
                ...
                counts.insert(0, '변수', c); res.append(counts)
            final_df = pd.concat(res)
            interpretation = "범주별 빈도와 유효 백분율을 확인하십시오."
        ...
        if st.button("실행"):
            ...
            assump_text = f"기대빈도 5 미만 셀 비율: {exp_p:.1f}%"; interpretation = f"📌 분석 결과 (χ²={chi2:.3f}, p={format_p(p)})"
        ...
        if st.button("실행") and len(df[g].unique()) == 2:
            ...
            assump_text, eq_v = check_parametric_assumptions([g1, g2], [str(gps[0]), str(gps[1])])
            stat, p = stats.ttest_ind(g1, g2, equal_var=eq_v)
            final_df = pd.DataFrame({"t-value": [stat], "df": [len(g1)+len(g2)-2], "p-value": [format_p(p)+get_stars(p)]})
            interpretation = f"📌 집단 간 평균 차이 유의성: p={format_p(p)}"
        ...
        if st.button("실행"):
            gps = [df[df[g]==val][y].dropna() for val in df[g].unique()]
            assump_text, eq_v = check_parametric_assumptions(gps, [f"Group {i}" for i in range(len(gps))])
            model = ols(f'{y} ~ C({g})', data=df).fit(); res = anova_lm(model, typ=2)
            final_df = res.reset_index().round(3); p = res.iloc[0,3]
            interpretation = f"📌 집단 간 차이 F={res.iloc[0,2]:.3f}, p={format_p(p)}."
        ...
        if st.button("실행") and len(vs) >= 2:
            ...
            plt.figure(figsize=(8,6)); sns.heatmap(final_df, annot=True, cmap="RdBu_r", vmin=-1, vmax=1); plot_img = get_plot_buffer()
            interpretation = "변수 간 선형적 관련성 강도를 확인하십시오."
        ...
        if st.button("실행") and len(vs) > 1:
            ...
            final_df = pd.DataFrame({"측정 지표": ["Cronbach's α"], "결과값": [f"{alpha:.3f}"]})
            interpretation = f"📌 도구의 신뢰도는 {alpha:.3f}로 {'적정' if alpha>=0.7 else '부족'}합니다."
        ...
        if st.button("실행") and xs:
            ...
            interpretation = f"📌 모형 설명력 R²={res.rsquared:.3f}, p={format_p(res.f_pvalue)}"
    ...
    if final_df is not None:
        ...
        if assump_text: 
            with st.expander("🔍 통계적 가정 검정 결과", expanded=True):
                st.markdown(f'<div class="assumption-box">{assump_text.replace("\n", "<br>")}</div>', unsafe_allow_html=True)
        
        c1, c2 = st.columns([1.5, 1])
        with c1: 
            st.table(final_df); st.info(interpretation)
            st.caption(WRITING_GUIDES.get(method, ""))
        with c2: 
            if plot_img: st.image(plot_img)
        
        st.download_button("📄 워드 리포트 다운로드", create_pro_report(method, final_df, interpretation, WRITING_GUIDES.get(method, ""), plot_b=plot_img, assump=assump_text), f"STATERA_{method}.docx")

else:
    st.markdown('<div style="text-align:center; padding:100px; color:#64748b;">데이터를 업로드하면 학술적 가이드와 분석 도구가 활성화됩니다.</div>', unsafe_allow_html=True)
